...
external build tree or an installed library. In either case there will be a
clean build of the library, so this may take several minutes to run.
...
def main() -> int:
    ...
    with TemporaryDirectory() as test_dir:
        ...
        if args.type.lower() == "source":
            ...
            defines["LIB_SOURCE_DIR"] = args.root
...
def _build(source: Path, build: Path, defs: dict, target=None):
    ...
    config_args = [f"-D{key}={str(value)}" for key, value in defs.items()]
